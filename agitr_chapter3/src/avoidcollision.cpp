...
void poseCallback(const turtlesim::Pose::ConstPtr& poseMsg, ros::Publisher& velPub)
{
    ...
    if (x >= SAFE_ZONE_X_MIN && x <= SAFE_ZONE_X_MAX && y >= SAFE_ZONE_Y_MIN && y <= SAFE_ZONE_Y_MAX)
    {
        ...
        if (x < SAFETY_MARGIN || x > 11.0 - SAFETY_MARGIN ||
            y < SAFETY_MARGIN || y > 11.0 - SAFETY_MARGIN)
        {
            // Set a linear and angular velocity for avoid collision
            velMsg.linear.x = 1;
            velMsg.angular.z = 1;
        } else {
    ...
    ROS_INFO_STREAM("Linear=" << velMsg.linear.x << " Angular=" << velMsg.angular.z);
}
